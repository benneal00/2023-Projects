...
SELECT 
    water_view, AVG(price) AS average_price
FROM
    housing
GROUP BY 1;
...
SELECT 
    county, water_view, AVG(days_on_market) AS avg_daysonmarket
FROM
    housing
GROUP BY 1 , 2;
...
SELECT ROUND(AVG(year_built), 0) AS average_year, AVG(price) AS average_price FROM housing
